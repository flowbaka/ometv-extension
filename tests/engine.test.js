...
function harness({ failHands = false } = {}) {
  let now = 10000, timer, resolveFace, handResult = null;
  const messages = [], listeners = {}, documentListeners = {}, metrics = { hands: 0, face: 0, loads: 0, inputs: [] };
  ...
  const context = { console: { log() {}, error() {} }, location: { origin: 'https://ome.tv' }, performance: { now: () => now }, Date: { now: () => now },
    ...
    Hands: class { setOptions() {} onResults(fn) { this.result = fn; } async initialize() { metrics.loads++; if (failHands) throw Error('test failure'); } async close() {} async send({ image }) { metrics.hands++; metrics.handSize = [image.width, image.height]; this.result(handResult || { multiHandLandmarks: [] }); } },
  ...
  return { metrics, messages, videos,
    async mode(mode, settings = {}) { listeners.message({ source, origin: 'https://ome.tv', data: { type: 'OMETV_EXT_CONFIG', base: 'chrome-extension://' + 'a'.repeat(32) + '/', mode, settings } }); await flush(); },
